            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('researchPublications.update', ['ED_ID' => $expertDomain->ED_ID, 'id' => $research->id]) }}">
                        ...
                        <div class="mb-4">
                            <label for="PB_Page" class="block text-sm font-medium text-gray-700">{{ __('Publication Pages') }}</label>
                            <input type="number" name="PB_Page" id="PB_Page" value="{{ old('PB_Page', $publication->PB_Page) }}" class="border rounded py-2 px-4 w-full">
                        </div>
                        <div class="mb-4">
                            <label for="PB_Detail" class="block text-sm font-medium text-gray-700">{{ __('Publication Details') }}</label>
                            <input type="text" name="PB_Detail" id="PB_Detail" value="{{ old('PB_Detail', $publication->PB_Detail) }}" class="border rounded py-2 px-4 w-full">
                        </div>
                        <div class="mb-4">
